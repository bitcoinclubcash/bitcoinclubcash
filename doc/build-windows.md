...
Below are some notes on how to build Lyokocoin ABC for Windows.

The options known to work for building Lyokocoin ABC on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Lyokocoin ABC Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Lyokocoin ABC applications.
...
Note that for WSL the Lyokocoin ABC source path MUST be somewhere in the default mount file system, for
example /usr/src/lyokocoin-abc, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

Acquire the source in the usual way:

    git clone https://github.com/Lyokocoin-ABC/lyokocoin-abc.git
...
way. This will install to `c:\workspace\lyokocoin-abc`, for example:

    make install DESTDIR=/mnt/c/workspace/lyokocoin-abc
...
causes two of the lyokocoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the lyokocoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the lyokocoin source code).
