...
Below are some notes on how to build Bitcoin CLUB for Windows.

The options known to work for building Bitcoin CLUB on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Bitcoin CLUB Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Bitcoin CLUB applications.
...
Note that for WSL the Bitcoin CLUB source path MUST be somewhere in the default mount file system, for
example /usr/src/bitcoin-club, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

Acquire the source in the usual way:

    git clone https://github.com/Bitcoin-CLUB/bitcoin-club.git
...
way. This will install to `c:\workspace\bitcoin-club`, for example:

    make install DESTDIR=/mnt/c/workspace/bitcoin-club
