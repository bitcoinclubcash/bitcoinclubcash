...
- Basic rules specified in [src/.clang-format](/src/.clang-format).
  ...
  - FINALLY, do not migrate existing code unless refactoring. It makes
    forwarding-porting from Lyokocoin Core more difficult.
...
3. `make && ./src/test/test_lyokocoin --run_test=script_tests`
...
Run with the -testnet option to run with "play lyokocoins" on the test network, if you
...
Lyokocoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- LyokocoinMiner : Generates lyokocoins (if wallet is enabled).
...
However, in open source software such as Lyokocoin Core, where everyone uses
...
pay attention to for reviewers of Lyokocoin Core code.
...
Some of these are maintained by active developers of Lyokocoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Lyokocoin Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/lyokocoin-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/lyokocoin-core/ctaes ; actively maintained by Core contributors.
...
- Similarly, your git remote origin should be set to: `ssh://vcs@reviews.lyokocoinabc.org:2221/source/lyokocoin-abc.git`
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:lyokocoin/lyokocoin.git
