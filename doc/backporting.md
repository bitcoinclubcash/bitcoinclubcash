...
The official Lyokocoin-ABC guide to keeping @deadalnix happy while backporting 
code from Lyokocoin Core.

Identifying commits
-------------------

1. Check out a copy of a Satoshi Lyokocoin client somewhere on your machine.
2. Identify the subsystem you'd like to work on.
3. Tag the fork commit as `fork-commit`. Lyokocoin-ABC was forked from Lyokocoin Core
...
Before you begin backporting commits, you will need to add an additional remote to your Lyokocoin-ABC repo.
For Lyokocoin Core, this repository would be added as:

```
git remote add core git@github.com:lyokocoin/lyokocoin.git.
git fetch core
```
...
7. `arc diff` and at the bottom of the summary note: "Backport of Lyokocoin Core PR# <XXXXX>".
...
2. Perform steps 2 through 6 from "Backporting each diff individually" in repetition for each diff
   in a Lyokocoin Core pull request.
3. `git rebase -i origin/master` and squash the commits together.
4. `arc diff` and at the bottom of the summary note: "Backport of Lyokocoin Core PR# <XXXXX>".
