...
class LyokocoinTestFramework():
    """Base class for a lyokocoin test script.

    Individual lyokocoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave lyokocoinds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop lyokocoinds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing lyokocoind/lyokocoin-cli (default: %default)")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info(
                "Note: lyokocoinds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a lyokocoind test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple lyokocoind test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'lyokocoind exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "lyokocoind should have exited with an error"
                else:
                    assert_msg = "lyokocoind should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as lyokocoind's debug.log with microprecision (so log files can be concatenated and sorted)
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("LyokocoinRPC")
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            # Create cache directories, run lyokocoinds:
            for i in range(MAX_NODES):
                datadir = initialize_datadir(self.options.cachedir, i)
                args = [os.getenv("LYOKOCOIND", "lyokocoind"), "-server",
        ...
        for i in range(self.num_nodes):
            ...
            # Overwrite port/rpcport in lyokocoin.conf
...
class ComparisonTestFramework(LyokocoinTestFramework):
    """Test framework for doing p2p comparison testing

    Sets up some lyokocoind binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("LYOKOCOIND", "lyokocoind"),
                          help="lyokocoind binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("LYOKOCOIND", "lyokocoind"),
                          help="lyokocoind binary to use for reference nodes (if any)")
