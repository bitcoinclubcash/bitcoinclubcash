...
'''
Test plan:
- Start lyokocoind's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on lyokocoind side:
...
- Create lyokocoinds that connect to them
- Manipulate the lyokocoinds using addnode (onetry) an observe effects
...
from test_framework.test_framework import LyokocoinTestFramework
...
class ProxyTest(LyokocoinTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: lyokocoind's SOCKS5 implementation only sends atyp DOMAINNAME,
        ...
        if self.have_ipv6:
            ...
            assert(isinstance(cmd, Socks5Command))
            # Note: lyokocoind's SOCKS5 implementation only sends atyp
        ...
        if test_onion:
            ...
            node.addnode("lyokocoinostk4e4re.onion:8333", "onetry")
            cmd = proxies[2].queue.get()
            assert(isinstance(cmd, Socks5Command))
            assert_equal(cmd.atyp, AddressType.DOMAINNAME)
            assert_equal(cmd.addr, b"lyokocoinostk4e4re.onion")
