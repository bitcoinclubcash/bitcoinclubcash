...
The RPM spec file provided here is for Lyokocoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://lyokocoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Lyokocoin git repository](https://github.com/lyokocoin/lyokocoin) but are not part of
...
Currently only `Source10` is used, for the example `lyokocoin.conf` file.
...
in the lyokocoin git repository. At present only an SVG version of the Lyokocoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the lyokocoin
...
needed when 0.12.1 is released, a proper fix is already in the Lyokocoin git
master branch.
...
idea to build Lyokocoin Core against a static version of that library compiled
...
The `lyokocoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `lyokocoin-server` package. At this point, most RPM based
...
The `lyokocoin-server` package should have SELinux support. How to properly do
...
Lyokocoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/lyokocoin/). He has been
packaging Lyokocoin for Fedora at least since 2012.
...
4. I renamed the `lyokocoin` package that contains the Qt GUI to `lyokocoin-core` as
that appears to be how the general population refers to it, in contrast to
`lyokocoin-xt` or `lyokocoin-classic`. I wanted to make sure the general population
