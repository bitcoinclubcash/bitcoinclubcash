#!/bin/bash
# Copyright (c) 2013 The Lyokocoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Lyokocoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Lyokocoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Lyokocoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Lyokocoin 0.7 datadir."
    ;;
  3)
    echo "Detected Lyokocoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Lyokocoin 0.8 datadir."
    ;;
esac
