#!/usr/bin/env python3
# Copyright (c) 2013-2017 The Lyokocoin Core developers
...
# LOC/UAHF specification.
PATTERN_AGENT = re.compile(
    r"^(/LyokocoinABC:0.17.(\d+)\(\S+\)/|/LyokocoinXT:0.11\(\S+\)/|/BUCash:1.3.(\d+)\(\S+\)/)")
...
def parseline(line):
    ...
    if len(sline) < 11:
        return None
    # All LOC clients apart BU and Classic has a space in the useragent string
    if len(sline) == 13:
...
def main():
    ...
    # Filter out hosts with multiple lyokocoin ports, these are likely abusive
    ips = filtermultiport(ips)
    # Look up ASNs and limit results, both per ASN and globally.
    # TODO during this bootstrap phase we need any LOC full nodes
    # active on the network, uncomment the following line once the
    # LOC chain will be consolidated.
