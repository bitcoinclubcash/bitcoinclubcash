 ...
 * alphanumeric.
 */
#ifndef LYOKOCOIN_BASE58_H
#define LYOKOCOIN_BASE58_H
 ...
 */
class CLyokocoinSecret : public CBase58Data {
public:
    ...
    CLyokocoinSecret(const CKey &vchSecret) { SetKey(vchSecret); }
    CLyokocoinSecret() {}
};

template <typename K, int Size, CChainParams::Base58Type Type>
class CLyokocoinExtKeyBase : public CBase58Data {
public:
    ...
    CLyokocoinExtKeyBase(const K &key) { SetKey(key); }

    CLyokocoinExtKeyBase(const std::string &strBase58c) {
        SetString(strBase58c.c_str(), Params().Base58Prefix(Type).size());
    }

    CLyokocoinExtKeyBase() {}
};

typedef CLyokocoinExtKeyBase<CExtKey, BIP32_EXTKEY_SIZE,
                           CChainParams::EXT_SECRET_KEY>
    CLyokocoinExtKey;
typedef CLyokocoinExtKeyBase<CExtPubKey, BIP32_EXTKEY_SIZE,
