 ...
 * is shared between lyokocoind, and lyokocoin-qt and non-server tools.
 */
...
void Shutdown() {
    ...
    RenameThread("lyokocoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode) {
    ...
    strUsage += HelpMessageOpt(
        "-conf=<file>", strprintf(_("Specify configuration file (default: %s)"),
                                  LYOKOCOIN_CONF_FILENAME));
    if (mode == HMM_LYOKOCOIND) {
#if HAVE_DECL_DAEMON
        strUsage += HelpMessageOpt(
            "-daemon",
    ...
    strUsage += HelpMessageOpt(
        "-pid=<file>",
        strprintf(_("Specify pid file (default: %s)"), LYOKOCOIN_PID_FILENAME));
#endif
    strUsage += HelpMessageOpt(
        "-prune=<n>",
        strprintf(
...
std::string LicenseInfo() {
    const std::string URL_SOURCE_CODE =
        "<https://github.com/Lyokocoin-ABC/lyokocoin-abc>";
    const std::string URL_WEBSITE = "<https://www.lyokocoinabc.org>";
...
void ThreadImport(const Config &config, std::vector<fs::path> vImportFiles) {
    RenameThread("lyokocoin-loadblk");
...
/** Sanity checks
 *  Ensure that Lyokocoin is running in a usable environment with all
...
bool AppInitParameterInteraction(Config &config) {
    ...
    // Signal Lyokocoin Cash support.
    // TODO: remove some time after the hardfork when no longer needed
    // to differentiate the network nodes.
    nLocalServices = ServiceFlags(nLocalServices | NODE_LYOKOCOIN_CASH);
...
static bool LockDataDirectory(bool probeOnly) {
    ...
    // Make sure only a single Lyokocoin process is using the data directory.
...
bool AppInitMain(Config &config, boost::thread_group &threadGroup,
    ...
    LogPrintf(
        "Using config file %s\n",
        GetConfigFile(gArgs.GetArg("-conf", LYOKOCOIN_CONF_FILENAME)).string());
