...
#define LYOKOCOIN_SEED_NONCE 0x0539a019ca550825ULL
...
class CSeederNode {
    ...
    void PushVersion() {
        ...
        uint64_t nLocalNonce = LYOKOCOIN_SEED_NONCE;
        int64_t nLocalServices = 0;
        CService myService;
        CAddress me(myService, ServiceFlags(NODE_NETWORK | NODE_LYOKOCOIN_CASH));
        BeginMessage("version");
        int nBestHeight = GetRequireHeight();
        std::string ver = "/lyokocoin-cash-seeder:0.15/";
...
public:
    CSeederNode(const CService &ip, std::vector<CAddress> *vAddrIn)
        : vSend(SER_NETWORK, 0), vRecv(SER_NETWORK, 0), nHeaderStart(-1),
          ...
          you(ip, ServiceFlags(NODE_NETWORK | NODE_LYOKOCOIN_CASH)) {
