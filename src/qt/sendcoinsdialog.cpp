...
void SendCoinsDialog::on_sendButton_clicked() {
    ...
    processSendCoinsReturn(
        prepareStatus,
        LyokocoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
    ...
    for (const SendCoinsRecipient &rcp : currentTransaction.getRecipients()) {
        ...
        QString amount =
            "<b>" + LyokocoinUnits::formatHtmlWithUnit(
    ...
    if (txFee > Amount(0)) {
        ...
        questionString.append(LyokocoinUnits::formatHtmlWithUnit(
    ...
    for (LyokocoinUnits::Unit u : LyokocoinUnits::availableUnits()) {
        if (u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(
                LyokocoinUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(
        tr("Total Amount %1")
            .arg(LyokocoinUnits::formatHtmlWithUnit(
...
void SendCoinsDialog::coinControlChangeEdited(const QString &text) {
    if (model && model->getAddressTableModel()) {
        ...
        if (text.isEmpty()) {
            ...
            // Invalid address
            ui->labelCoinControlChangeLabel->setText(
                tr("Warning: Invalid Lyokocoin address"));
        } else {
            // Valid address
