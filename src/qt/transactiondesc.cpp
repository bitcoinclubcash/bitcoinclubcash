...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx,
    ...
    if (wtx.IsCoinBase() && nCredit == Amount(0)) {
        ...
        if (wtx.IsInMainChain())
            strHTML += LyokocoinUnits::formatHtmlWithUnit(unit, nUnmatured) +
        ...
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " +
                   LyokocoinUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    } else {
        ...
        if (fAllFromMe) {
            ...
            for (const CTxOut &txout : wtx.tx->vout) {
                ...
                strHTML +=
                    "<b>" + tr("Debit") + ":</b> " +
                    LyokocoinUnits::formatHtmlWithUnit(unit, -1 * txout.nValue) +
                    "<br>";
                if (toSelf)
                    strHTML +=
                        "<b>" + tr("Credit") + ":</b> " +
                        LyokocoinUnits::formatHtmlWithUnit(unit, txout.nValue) +
                        "<br>";
            }
            ...
            if (fAllToMe) {
                ...
                strHTML += "<b>" + tr("Total debit") + ":</b> " +
                           LyokocoinUnits::formatHtmlWithUnit(unit, -1 * nValue) +
                           "<br>";
                strHTML += "<b>" + tr("Total credit") + ":</b> " +
                           LyokocoinUnits::formatHtmlWithUnit(unit, nValue) +
                           "<br>";
            }
            ...
            if (nTxFee > Amount(0))
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " +
                           LyokocoinUnits::formatHtmlWithUnit(unit, -1 * nTxFee) +
                           "<br>";
        } else {
            //
            ...
            for (const CTxIn &txin : wtx.tx->vin) {
                if (wallet->IsMine(txin))
                    strHTML +=
                        "<b>" + tr("Debit") + ":</b> " +
                        LyokocoinUnits::formatHtmlWithUnit(
    ...
    strHTML += "<b>" + tr("Net amount") + ":</b> " +
               LyokocoinUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
    ...
    // Message from normal lyokocoincash:URI (lyokocoincash:123...?message=example)
        ...
        for (const CTxIn &txin : wtx.tx->vin) {
            ...
            if (pcoinsTip->GetCoin(prevout, prev)) {
                ...
                strHTML = strHTML + " " + tr("Amount") + "=" +
                          LyokocoinUnits::formatHtmlWithUnit(unit, vout.nValue);
                strHTML = strHTML +
