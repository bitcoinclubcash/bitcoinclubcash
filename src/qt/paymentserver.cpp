...
const int LYOKOCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
// BIP70 payment protocol messages
const char *BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char *BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char *BIP71_MIMETYPE_PAYMENT = "application/lyokocoincash-payment";
const char *BIP71_MIMETYPE_PAYMENTACK = "application/lyokocoincash-paymentack";
const char *BIP71_MIMETYPE_PAYMENTREQUEST =
    "application/lyokocoincash-paymentrequest";
...
static std::string ipcParseURI(const QString &arg, const CChainParams &params,
                               bool useCashAddr) {
    const QString scheme = GUIUtil::lyokocoinURIScheme(params, useCashAddr);
    if (!arg.startsWith(scheme + ":", Qt::CaseInsensitive)) {
        return {};
    }

    SendCoinsRecipient r;
    if (!GUIUtil::parseLyokocoinURI(scheme, arg, &r)) {
        return {};
    }
...
bool PaymentServer::ipcSendCommandLine() {
    ...
    for (const QString &r : savedPaymentRequests) {
        ...
        if (!socket->waitForConnected(LYOKOCOIN_IPC_CONNECT_TIMEOUT)) {
            delete socket;
            socket = nullptr;
            return false;
        }
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(LYOKOCOIN_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QObject *parent, bool startLocalServer)
    ...
    // on Mac: sent when you click lyokocoincash: links
...
//
// OSX-specific way of handling lyokocoincash: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager() {
    ...
    // netManager is used to fetch paymentrequests given in lyokocoincash: URIs
...
bool PaymentServer::handleURI(const QString &scheme, const QString &s) {
    ...
    if (GUIUtil::parseLyokocoinURI(scheme, s, &recipient)) {
        ...
        Q_EMIT message(
            ...
            tr("URI cannot be parsed! This can be caused by an invalid "
               "Lyokocoin address or malformed URI parameters."),
            CClientUIInterface::ICON_WARNING);
    }
...
void PaymentServer::handleURIOrFile(const QString &s) {
    ...
    // lyokocoincash: CashAddr URI
    QString schemeCash = GUIUtil::lyokocoinURIScheme(Params(), true);
    if (handleURI(schemeCash, s)) {
        return;
    }

    // lyokocoincash: Legacy URI
    QString schemeLegacy = GUIUtil::lyokocoinURIScheme(Params(), false);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus &request,
    ...
    for (const std::pair<CScript, Amount> &sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom lyokocoin addresses are
            // not supported
        ...
        // Lyokocoin amounts are stored as (optional) uint64 in the protobuf
        ...
        if (txOut.IsDust(dustRelayFee)) {
            Q_EMIT message(
                ...
                tr("Requested payment amount of %1 is too small (considered "
                   "dust).")
                    .arg(LyokocoinUnits::formatWithUnit(
