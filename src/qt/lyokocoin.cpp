...
static std::string Translate(const char *psz) {
    return QCoreApplication::translate("lyokocoin-abc", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase,
    ...
    // Load e.g. lyokocoin_de.qm (shortcut "de" needs to be defined in
    // lyokocoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. lyokocoin_de_DE.qm (shortcut "de_DE" needs to be defined in
    // lyokocoin.qrc)
...
/**
 * Class encapsulating Lyokocoin ABC startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class LyokocoinABC : public QObject {
    Q_OBJECT
public:
    explicit LyokocoinABC();
...
/** Main Lyokocoin application object */
class LyokocoinApplication : public QApplication {
    Q_OBJECT
public:
    explicit LyokocoinApplication(int &argc, char **argv);
    ~LyokocoinApplication();
    ...
    /// Get window identifier of QMainWindow (LyokocoinGUI)
...
#include "lyokocoin.moc"

LyokocoinABC::LyokocoinABC() : QObject() {}

void LyokocoinABC::handleRunawayException(const std::exception *e) {
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(GetWarnings("gui")));
}

void LyokocoinABC::initialize(Config *cfg) {
    Config &config(*cfg);
    try {
...
LyokocoinApplication::LyokocoinApplication(int &argc, char **argv)
    ...
    // This must be done inside the LyokocoinApplication constructor, or after it,
    // because PlatformStyle::instantiate requires a QApplication.
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", LyokocoinGUI::DEFAULT_UIPLATFORM);
    platformStyle =
...
LyokocoinApplication::~LyokocoinApplication() {
    if (coreThread) {
...
#ifdef ENABLE_WALLET
void LyokocoinApplication::createPaymentServer() {
    paymentServer = new PaymentServer(this);
}
#endif

void LyokocoinApplication::createOptionsModel(bool resetSettings) {
    optionsModel = new OptionsModel(nullptr, resetSettings);
}

void LyokocoinApplication::createWindow(const Config *config,
                                      const NetworkStyle *networkStyle) {
    window = new LyokocoinGUI(config, platformStyle, networkStyle, 0);
...
void LyokocoinApplication::createSplashScreen(const NetworkStyle *networkStyle) {
...
void LyokocoinApplication::startThread() {
    if (coreThread) return;
    coreThread = new QThread(this);
    LyokocoinABC *executor = new LyokocoinABC();
...
void LyokocoinApplication::parameterSetup() {
    InitLogging();
    InitParameterInteraction();
}

void LyokocoinApplication::requestInitialize(Config &config) {
...
void LyokocoinApplication::requestShutdown(Config &config) {
...
void LyokocoinApplication::initializeResult(int retval) {
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(LyokocoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(LyokocoinGUI::DEFAULT_WALLET);
        ...
        // lyokocoincash: URIs or payment requests:
...
void LyokocoinApplication::shutdownResult(int retval) {
    qDebug() << __func__ << ": Shutdown result: " << retval;
    // Exit main loop after shutdown finished.
    quit();
}

void LyokocoinApplication::handleRunawayException(const QString &message) {
    QMessageBox::critical(
        0, "Runaway exception",
        LyokocoinGUI::tr("A fatal error occurred. Lyokocoin can no longer continue "
                       "safely and will quit.") +
            QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}

WId LyokocoinApplication::getMainWinId() const {
    if (!window) return 0;

    return window->winId();
}

#ifndef LYOKOCOIN_QT_TEST

static void MigrateSettings() {
    assert(!QApplication::applicationName().isEmpty());

    static const QString legacyAppName("Lyokocoin-Qt"),
#ifdef Q_OS_DARWIN
    ...
    // We only migrate settings if we have Core settings but no Lyokocoin-ABC
    // settings
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    Q_INIT_RESOURCE(lyokocoin);
    Q_INIT_RESOURCE(lyokocoin_locale);

    LyokocoinApplication app(argc, argv);
    ...
    // Migrate settings from core's/our old GUI settings to Lyokocoin ABC
    // only if core's exist but Lyokocoin ABC's doesn't.
    ...
    /// 6. Determine availability of data directory and parse lyokocoin.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", LYOKOCOIN_CONF_FILENAME));
    } catch (const std::exception &e) {
        QMessageBox::critical(
            0, QObject::tr(PACKAGE_NAME),
    ...
    // lyokocoincash: links repeatedly have their payment requests routed to this
    // process:
