...
const std::string LyokocoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
    "macosx"
 ...
 */
const QString LyokocoinGUI::DEFAULT_WALLET = "~Default";

LyokocoinGUI::LyokocoinGUI(const Config *cfg, const PlatformStyle *_platformStyle,
...
void LyokocoinGUI::createActions() {
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Lyokocoin address"));
    ...
    receiveCoinsAction->setStatusTip(
        tr("Request payments (generates QR codes and %1: URIs)")
            .arg(GUIUtil::lyokocoinURIScheme(*cfg)));
    ...
    signMessageAction->setStatusTip(
        tr("Sign messages with your Lyokocoin addresses to prove you own them"));
    verifyMessageAction =
        new QAction(platformStyle->TextColorIcon(":/icons/verify"),
                    tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(
        tr("Verify messages to ensure they were signed with specified Lyokocoin "
           "addresses"));
    ...
    showHelpMessageAction->setStatusTip(
        tr("Show the %1 help message to get a list with possible Lyokocoin "
...
void LyokocoinGUI::createMenuBar() {
#ifdef Q_OS_MAC
...
void LyokocoinGUI::createToolBars() {
    if (walletFrame) {
...
void LyokocoinGUI::setClientModel(ClientModel *_clientModel) {
...
#ifdef ENABLE_WALLET
bool LyokocoinGUI::addWallet(const QString &name, WalletModel *walletModel) {
    if (!walletFrame) return false;
    setWalletActionsEnabled(true);
    return walletFrame->addWallet(name, walletModel);
}

bool LyokocoinGUI::setCurrentWallet(const QString &name) {
    if (!walletFrame) return false;
    return walletFrame->setCurrentWallet(name);
}

void LyokocoinGUI::removeAllWallets() {
...
void LyokocoinGUI::setWalletActionsEnabled(bool enabled) {
...
void LyokocoinGUI::createTrayIcon(const NetworkStyle *networkStyle) {
#ifndef Q_OS_MAC
...
void LyokocoinGUI::createTrayIconMenu() {
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void LyokocoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason) {
...
void LyokocoinGUI::optionsClicked() {
...
void LyokocoinGUI::aboutClicked() {
...
void LyokocoinGUI::showDebugWindow() {
...
void LyokocoinGUI::showDebugWindowActivateConsole() {
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void LyokocoinGUI::showHelpMessageClicked() {
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void LyokocoinGUI::openClicked() {
...
void LyokocoinGUI::gotoOverviewPage() {
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void LyokocoinGUI::gotoHistoryPage() {
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void LyokocoinGUI::gotoReceiveCoinsPage() {
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void LyokocoinGUI::gotoSendCoinsPage(QString addr) {
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void LyokocoinGUI::gotoSignMessageTab(QString addr) {
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void LyokocoinGUI::gotoVerifyMessageTab(QString addr) {
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void LyokocoinGUI::updateNetworkState() {
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Lyokocoin network", "", count) +
...
void LyokocoinGUI::setNumConnections(int count) {
    updateNetworkState();
}

void LyokocoinGUI::setNetworkActive(bool networkActive) {
    updateNetworkState();
}

void LyokocoinGUI::updateHeadersSyncProgressLabel() {
...
void LyokocoinGUI::setNumBlocks(int count, const QDateTime &blockDate,
...
void LyokocoinGUI::message(const QString &title, const QString &message,
                         unsigned int style, bool *ret) {
    // default title
    QString strTitle = tr("Lyokocoin");
...
void LyokocoinGUI::changeEvent(QEvent *e) {
...
void LyokocoinGUI::closeEvent(QCloseEvent *event) {
#ifndef Q_OS_MAC // Ignored on Mac
    if (clientModel && clientModel->getOptionsModel()) {
...
void LyokocoinGUI::showEvent(QShowEvent *event) {
...
#ifdef ENABLE_WALLET
void LyokocoinGUI::incomingTransaction(const QString &date, int unit,
    ...
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n")
                      .arg(LyokocoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void LyokocoinGUI::dragEnterEvent(QDragEnterEvent *event) {
    // Accept only URIs
    if (event->mimeData()->hasUrls()) event->acceptProposedAction();
}

void LyokocoinGUI::dropEvent(QDropEvent *event) {
...
bool LyokocoinGUI::eventFilter(QObject *object, QEvent *event) {
...
#ifdef ENABLE_WALLET
bool LyokocoinGUI::handlePaymentRequest(const SendCoinsRecipient &recipient) {
    // URI has to be valid
...
void LyokocoinGUI::setHDStatus(int hdEnabled) {
...
void LyokocoinGUI::setEncryptionStatus(int status) {
    switch (status) {
...
void LyokocoinGUI::showNormalIfMinimized(bool fToggleHidden) {
    if (!clientModel) return;
...
void LyokocoinGUI::toggleHidden() {
    showNormalIfMinimized(true);
}

void LyokocoinGUI::detectShutdown() {
...
void LyokocoinGUI::showProgress(const QString &title, int nProgress) {
    if (nProgress == 0) {
...
void LyokocoinGUI::setTrayIconVisible(bool fHideTrayIcon) {
    if (trayIcon) {
        trayIcon->setVisible(!fHideTrayIcon);
    }
}

void LyokocoinGUI::showModalOverlay() {
    if (modalOverlay &&
        (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(LyokocoinGUI *gui, const std::string &message,
...
void LyokocoinGUI::subscribeToCoreSignals() {
...
void LyokocoinGUI::unsubscribeFromCoreSignals() {
...
void LyokocoinGUI::toggleNetworkActive() {
    if (clientModel) {
...
UnitDisplayStatusBarControl::UnitDisplayStatusBarControl(
    ...
    QList<LyokocoinUnits::Unit> units = LyokocoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const LyokocoinUnits::Unit unit : units) {
        max_width = qMax(max_width, fm.width(LyokocoinUnits::name(unit)));
    }
...
void UnitDisplayStatusBarControl::createContextMenu() {
    menu = new QMenu(this);
    for (LyokocoinUnits::Unit u : LyokocoinUnits::availableUnits()) {
        QAction *menuAction = new QAction(QString(LyokocoinUnits::name(u)), this);
...
void UnitDisplayStatusBarControl::updateDisplayUnit(int newUnits) {
    setText(LyokocoinUnits::name(newUnits));
}
