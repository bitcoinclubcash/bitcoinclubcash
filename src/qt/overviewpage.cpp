...
public:
    TxViewDelegate(const PlatformStyle *_platformStyle,
        ...
        : QAbstractItemDelegate(parent), unit(LyokocoinUnits::LOC),
    ...
    inline void paint(QPainter *painter, const QStyleOptionViewItem &option,
        ...
        painter->setPen(foreground);
        QString amountText = LyokocoinUnits::formatWithUnit(
            unit, amount, true, LyokocoinUnits::separatorAlways);
        if (!confirmed) {
...
void OverviewPage::setBalance(const Amount balance,
    ...
    ui->labelBalance->setText(LyokocoinUnits::formatWithUnit(
        unit, balance, false, LyokocoinUnits::separatorAlways));
    ui->labelUnconfirmed->setText(LyokocoinUnits::formatWithUnit(
        unit, unconfirmedBalance, false, LyokocoinUnits::separatorAlways));
    ui->labelImmature->setText(LyokocoinUnits::formatWithUnit(
        unit, immatureBalance, false, LyokocoinUnits::separatorAlways));
    ui->labelTotal->setText(LyokocoinUnits::formatWithUnit(
        unit, balance + unconfirmedBalance + immatureBalance, false,
        LyokocoinUnits::separatorAlways));
    ui->labelWatchAvailable->setText(LyokocoinUnits::formatWithUnit(
        unit, watchOnlyBalance, false, LyokocoinUnits::separatorAlways));
    ui->labelWatchPending->setText(LyokocoinUnits::formatWithUnit(
        unit, watchUnconfBalance, false, LyokocoinUnits::separatorAlways));
    ui->labelWatchImmature->setText(LyokocoinUnits::formatWithUnit(
        unit, watchImmatureBalance, false, LyokocoinUnits::separatorAlways));
    ui->labelWatchTotal->setText(LyokocoinUnits::formatWithUnit(
        unit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance,
        false, LyokocoinUnits::separatorAlways));
...
void OverviewPage::setWalletModel(WalletModel *model) {
    ...
    // update the display unit, to not use the default ("LOC")
    updateDisplayUnit();
}
