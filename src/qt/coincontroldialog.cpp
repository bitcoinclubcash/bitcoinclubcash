...
// click on header
    ...
    // see https://github.com/lyokocoin/lyokocoin/issues/5716
...
void CoinControlDialog::copyAmount() {
    GUIUtil::setClipboard(
        LyokocoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog) {
    ...
    int nDisplayUnit = LyokocoinUnits::LOC;
    ...
    // Amount
    l2->setText(LyokocoinUnits::formatWithUnit(nDisplayUnit, nAmount));
    // Fee
    l3->setText(LyokocoinUnits::formatWithUnit(nDisplayUnit, nPayFee));
    // After Fee
    l4->setText(LyokocoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));
    // Bytes
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));
    // Dust
    l7->setText(fDust ? tr("yes") : tr("no"));
    // Change
    l8->setText(LyokocoinUnits::formatWithUnit(nDisplayUnit, nChange));
    if (nPayFee > Amount(0) && (coinControl->nMinimumTotalFee < nPayFee)) {
    ...
    for (const std::pair<QString, std::vector<COutput>> &coins : mapCoins) {
        ...
        for (const COutput &out : coins.second) {
            ...
            if (ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey,
                ...
                // if listMode or change => show lyokocoin address. In tree mode,
            ...
            itemOutput->setText(
                COLUMN_AMOUNT,
                LyokocoinUnits::format(nDisplayUnit,
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(
                COLUMN_AMOUNT, LyokocoinUnits::format(nDisplayUnit, nSum));
