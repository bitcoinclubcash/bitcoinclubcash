 ...
 *
 * This is the developer documentation of Lyokocoin ABC
 * (https://www.lyokocoinabc.org/). Lyokocoin ABC is a client for the digital
 * currency called Lyokocoin Cash (https://www.lyokocoincash.org/), which enables
 * instant payments to anyone, anywhere in the world. Lyokocoin Cash uses
...
bool AppInit(int argc, char *argv[]) {
    ...
    // Parameters
    //
    // If Qt is used, parameters/lyokocoin.conf are parsed in qt/lyokocoin.cpp's
    // main()
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) &&
                !boost::algorithm::istarts_with(argv[i], "lyokocoin:"))
                fCommandLine = true;

        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in "
                            "lyokocoind anymore. Use the lyokocoin-cli utility "
                            "instead.\n");
            exit(EXIT_FAILURE);
        }
        // -server defaults to true for lyokocoind but not for the GUI so do this
        // here
...
#if HAVE_DECL_DAEMON
            fprintf(stdout, "Lyokocoin server starting\n");
...
int main(int argc, char *argv[]) {
    ...
    // Connect lyokocoind signal handlers
    noui_connect();
