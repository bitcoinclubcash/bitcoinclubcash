...
static UniValue getnewaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            ...
            "\nReturns a new Lyokocoin address for receiving payments.\n"
            ...
            "the given name.\n"
            "\nResult:\n"
            "\"address\"    (string) The new lyokocoin address\n"
            "\nExamples:\n" +
...
static UniValue getaccountaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current Lyokocoin address for receiving "
            ...
            "the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account lyokocoin address\n"
            "\nExamples:\n" +
...
static UniValue getrawchangeaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Lyokocoin address, for receiving change.\n"
...
static UniValue setaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "address.\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The lyokocoin address to "
...
static UniValue getaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "address.\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The lyokocoin address for "
            "account lookup.\n"
            "\nResult:\n"
...
static UniValue getaddressesbyaccount(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a lyokocoin address associated with "
            "the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n" +
...
static UniValue sendtoaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The lyokocoin address "
            "to send to.\n"
            ...
            "lyokocoins than you enter in the amount field.\n"
            "\nResult:\n"
...
static UniValue listaddressgroupings(const Config &config,
    ...
    if (request.fHelp) {
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The lyokocoin address\n"
...
static UniValue sendfrom(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 3 ||
        ...
        throw std::runtime_error(
            ...
            "reflect\n"
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The lyokocoin address "
            "to send funds to.\n"
...
static UniValue sendmany(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The lyokocoin "
            ...
            "lyokocoins than you enter in their corresponding amount field.\n"
    ...
    for (const std::string &name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid Lyokocoin address: ") +
                                   name_);
        }
...
static UniValue addmultisigaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "Each key is a Lyokocoin address or hex-encoded public key.\n"
            ...
            "2. \"keys\"         (string, required) A json array of lyokocoin "
            "addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) lyokocoin address or hex-encoded "
            "public key\n"
            "       ...,\n"
            "     ]\n"
...
static UniValue listtransactions(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The lyokocoin address of "
...
static UniValue gettransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "default account.\n"
            "      \"address\" : \"address\",          (string) The lyokocoin "
...
static UniValue walletpassphrase(const Config &config,
    ...
    if (pwallet->IsCrypted() && (request.fHelp || request.params.size() != 2)) {
        throw std::runtime_error(
            ...
            "private keys such as sending lyokocoins\n"
            "\nArguments:\n"
...
static UniValue encryptwallet(const Config &config,
    ...
    StartShutdown();
    return "wallet encrypted; Lyokocoin server stopping, restart to run with "
...
static UniValue lockunspent(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "selection, when spending lyokocoins.\n"
...
static UniValue listunspent(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of lyokocoin addresses to "
            "filter\n"
            "    [\n"
            "      \"address\"   (string) lyokocoin address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the lyokocoin address\n"
...
static UniValue fundrawtransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "address) The lyokocoin address to receive the change\n"
            ...
            if (options.exists("changeAddress")) {
                ...
                if (!IsValidDestination(dest)) {
                    throw JSONRPCError(
                        ...
                        "changeAddress must be a valid lyokocoin address");
                }
