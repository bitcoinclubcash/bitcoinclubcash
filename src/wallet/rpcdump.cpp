...
UniValue importprivkey(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            "importprivkey \"lyokocoinprivkey\" ( \"label\" ) ( rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your "
            "wallet.\n"
            "\nArguments:\n"
            "1. \"lyokocoinprivkey\"   (string, required) The private key (see "
            "dumpprivkey)\n"
...
UniValue importaddress(const Config &config, const JSONRPCRequest &request) {
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                           "Invalid Lyokocoin address or script");
    }
...
UniValue dumpprivkey(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The lyokocoin address for the "
            "private key\n"
            "\nResult:\n"
    ...
    return CLyokocoinSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "(either absolute or relative to lyokocoind)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/lyokocoin/lyokocoin/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by Lyokocoin %s\n", CLIENT_BUILD);
    ...
    for (std::vector<std::pair<int64_t, CKeyID>>::const_iterator it =
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CLyokocoinSecret(key).ToString(),
                              strTime);
