// Copyright (c) 2013-2015 The Lyokocoin Core developers
...
TestVector test1 = TestVector("000102030405060708090a0b0c0d0e0f")(
    ...
    0x80000000)("xpub68Gmy5EdvgibQVfPdqkBLOCxA5htiqg55crXYuXoQRKfDBFA1WEjWgP6LH"
...
void RunTest(const TestVector &test) {
    ...
    for (const TestDerivation &derive : test.vDerive) {
        ...
        // Test private key
        CLyokocoinExtKey b58key;
        b58key.SetKey(key);
        BOOST_CHECK(b58key.ToString() == derive.prv);

        CLyokocoinExtKey b58keyDecodeCheck(derive.prv);
        CExtKey checkKey = b58keyDecodeCheck.GetKey();
        // ensure a base58 decoded key also matches
        assert(checkKey == key);

        // Test public key
        CLyokocoinExtPubKey b58pubkey;
        b58pubkey.SetKey(pubkey);
        BOOST_CHECK(b58pubkey.ToString() == derive.pub);

        CLyokocoinExtPubKey b58PubkeyDecodeCheck(derive.pub);
