...
static int AppInitRawTx(int argc, char *argv[]) {
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
        ...
        std::string strUsage =
            strprintf(_("%s lyokocoin-tx utility version"), _(PACKAGE_NAME)) +
            " " + FormatFullVersion() + "\n\n" + _("Usage:") + "\n" +
            "  lyokocoin-tx [options] <hex-tx> [commands]  " +
            _("Update hex-encoded lyokocoin transaction") + "\n" +
            "  lyokocoin-tx [options] -create [commands]   " +
            _("Create hex-encoded lyokocoin transaction") + "\n" + "\n";
...
static int CommandLineRawTx(int argc, char *argv[],
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded lyokocoin transaction
