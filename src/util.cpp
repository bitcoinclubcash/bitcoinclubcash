...
const char *const LYOKOCOIN_CONF_FILENAME = "lyokocoin.conf";
const char *const LYOKOCOIN_PID_FILENAME = "lyokocoind.pid";
...
#else
    const char *pszModule = "lyokocoin";
#endif
    if (pex)
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Lyokocoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Lyokocoin
// Mac: ~/Library/Application Support/Lyokocoin
// Unix: ~/.lyokocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Lyokocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Lyokocoin";
#else
    // Unix
    return pathRet / ".lyokocoin";
#endif
#endif
}
...
#ifndef WIN32
fs::path GetPidFile() {
    fs::path pathPidFile(gArgs.GetArg("-pid", LYOKOCOIN_PID_FILENAME));
